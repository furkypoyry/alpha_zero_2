...
from alpha_zero.core.mcts_eg import Node, parallel_uct_search, uct_search
...
from alpha_zero.core.replay_eg import UniformReplay, Transition
...
def play_and_record_one_game(
    ...
    while not done:  # For each step
        ...
        # Compute per-action child value cache from root_node to support IG loss later.
        # Use NaN where child wasn't expanded.
        num_actions = env.action_dim
        ig_child_v = np.full((num_actions,), np.nan, dtype=np.float32)
        if root_node is not None and hasattr(root_node, 'children'):
            for a, child in root_node.children.items():
                # value_net is in [-1, 1] (raw network output)
                if getattr(child, 'value_net', None) is not None:
                    ig_child_v[a] = float(child.value_net)
        episode_ig_child_vs.append(ig_child_v)
    ...
    game_seq = [
        Transition(state=x, pi_prob=pi, value=v, ig_child_v=ig)
        for x, pi, v, ig in zip(episode_states, episode_search_pis, episode_values, episode_ig_child_vs)
    ]
        ...
        try:
            ...
            if replay.num_games_added == min_games or (
                ...
                while training_steps < target_t:
                    ...
                    optimizer.zero_grad()
                    pi_loss, v_loss, ig_loss = compute_losses(
                        network, device, transitions, argument_data, ig_alpha
                    )
                    loss = pi_loss + v_loss + ig_alpha * ig_loss
                    loss.backward()
                    optimizer.step()
                    lr_scheduler.step()
                    training_steps += 1
...
def compute_losses(
    ...
    # Information Gain auxiliary loss: L_IG = - sum_a p(a|s) * IG(s,a)
    # where IG(s,a) = (1 - v') * log(1 - v') - v * log(v), with v, v' in [0,1]
    ig_loss = torch.tensor(0.0, device=device, dtype=torch.float32)
    if ig_alpha and getattr(transitions, 'ig_child_v', None) is not None:
        # predicted probabilities for actions
        pred_pi = torch.softmax(pred_pi_logits, dim=-1)
        # parent v in [0,1]
        v_parent = torch.clamp((pred_v.squeeze(-1) + 1.0) / 2.0, 1e-6, 1.0 - 1e-6)  # [B]
        # child v' raw in [-1,1] from transitions
        child_v_raw = torch.from_numpy(transitions.ig_child_v).to(device=device, dtype=torch.float32)  # [B, A]
        # mask NaNs (unexpanded children)
        mask = torch.isfinite(child_v_raw)  # [B, A]
        v_child = torch.clamp((child_v_raw + 1.0) / 2.0, 1e-6, 1.0 - 1e-6)  # [B, A]
        v_parent_expanded = v_parent.unsqueeze(-1).expand_as(v_child)  # [B, A]
        # Compute IG(s,a) only where mask is True to avoid NaN issues
        ig_sa = torch.zeros_like(v_child)
        valid_idx = mask
        if valid_idx.any():
            vc = v_child[valid_idx]
            vp = v_parent_expanded[valid_idx]
            ig_vals = (1.0 - vc) * torch.log(1.0 - vc) - (vp * torch.log(vp))
            ig_sa[valid_idx] = ig_vals
        # L_IG per-sample
        l_ig_sample = -(pred_pi * ig_sa).sum(dim=-1)
        ig_loss = l_ig_sample.mean()

    return policy_loss, value_loss, ig_loss
