...
class Node:
    ...
    def __init__(
        ...
        # Cached neural net value prediction for this state (from current player's perspective).
        # AlphaZero networks output value in [-1, 1]; we'll cache that raw value.
        self.value_net = None
...
def best_child(
    ...
    # Optional Entropy-Guided bonus: IG(s,a) = (1 - v') * log(1 - v') - v * log(v),
    # where v and v' are in [0,1]. Our cached network values are in [-1,1], so map via (x+1)/2.
    if eg_lambda and eg_lambda != 0.0 and node.value_net is not None:
        eps = 1e-6
        # Parent v in [0,1]
        v_parent = (float(node.value_net) + 1.0) / 2.0
        v_parent = min(max(v_parent, eps), 1.0 - eps)
        ig_scores = np.zeros_like(ucb_scores, dtype=np.float32)
        # Iterate over actions in the same order as children arrays
        # We need index alignment: children stats arrays are aligned by action index 0..num_actions-1
        for a in range(node.num_actions):
            child = node.children.get(a)
            if child is None or not child.is_expanded or child.value_net is None:
                ig = 0.0
            else:
                v_child = (float(child.value_net) + 1.0) / 2.0
                v_child = min(max(v_child, eps), 1.0 - eps)
                ig = (1.0 - v_child) * math.log(1.0 - v_child) - v_parent * math.log(v_parent)
            ig_scores[a] = ig
        ucb_scores = ucb_scores + (eg_lambda * ig_scores)
...
def uct_search(
    ...
    if root_node is None:
        ...
        root_node.value_net = float(value)
