...
class Transition(NamedTuple):
    ...
    # Optional: per-action child state values v'(s') from MCTS (raw in [-1, 1]).
    # Use NaN for actions where child wasn't expanded; this lets training mask them.
    ig_child_v: Optional[np.ndarray] = None


TransitionStructure = Transition(state=None, pi_prob=None, value=None, ig_child_v=None)
...
class UniformReplay:
    ...
    def encoder(self, transition: Transition) -> Transition:
        if self.compress_data:
            ig_child_v = (
                compress_array(transition.ig_child_v) if transition.ig_child_v is not None else None
            )
            return transition._replace(
                state=compress_array(transition.state),
                ig_child_v=ig_child_v,
            )
        return transition

    def decoder(self, transition: Transition) -> Transition:
        if self.compress_data:
            ig_child_v = uncompress_array(transition.ig_child_v) if transition.ig_child_v is not None else None
            return transition._replace(
